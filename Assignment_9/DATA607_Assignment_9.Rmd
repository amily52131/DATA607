...
Sign up for the nytimes developer apis and activate an API key for the API accessing. I choose the books API and from the API documentation get the base url and use it to build out API call. 
```{r}
...
booknames <- json_data$results %>% as.data.frame() %>% print()
```
